...
namespace mls
{
    ...
    LlmSession::LlmSession(std::string model_path, json config, json extra_config, std::vector<std::string> history) : model_path_(std::move(model_path)), config_(std::move(config)), extra_config_(std::move(extra_config))
    {
...
// #define USE_DEBUG_PROMOT
    ...
    bool LlmSession::initializeLlmForBenchmark(BenchmarkResult &result, const BenchmarkCallback &callback)
    {
        ...
        if (!llm)
        {
            ...
            if (callback.onError)
                callback.onError(result.error_message);
            return false;
        }
                                    ...
                                    BenchmarkResult &result, const BenchmarkCallback &callback)
    {
        ...
        if (iteration > 0)
        { // Exclude the first performance value
